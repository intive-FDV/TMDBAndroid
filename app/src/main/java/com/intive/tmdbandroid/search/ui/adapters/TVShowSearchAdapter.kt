...
class TVShowSearchAdapter : PagingDataAdapter<TVShow, TVShowSearchAdapter.TVShowHolder>(REPO_COMPARATOR) {
    companion object {
        ...
        with(holder) {
            with(getItem(position) as TVShow) {
                ...
                try {
                    ...
                    binding.itemYearSearch.text = stringDate
                } catch (e: Exception) {
                    binding.itemYearSearch.text = ""
                }

//                binding.itemRatingSearch.progress = percentage
...
//                }
                ...
                binding.itemRatingSearch.numStars = percentage
                binding.itemTitleSearch.text = name
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TVShowHolder = TVShowHolder(
        ItemFoundSearchBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    )

    inner class TVShowHolder (val binding: ItemFoundSearchBinding) : RecyclerView.ViewHolder(binding.root)
}